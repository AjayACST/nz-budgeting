...
//         const lastRefresh = DateTime.fromISO(account.refreshed?.balance || '').toFormat('yyyy-LL-dd HH:mm:ss')
//         const account_data = [account._id, account.name, account.formatted_account, account.balance?.current, account.balance?.available, process.env.KINDE_USER_TOKEN, account.connection._id, lastRefresh, lastRefresh]
//         await connection.query('INSERT INTO accounts VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE last_refresh = ?', account_data)
//
